...
// Define OpenGL 1.2 constant if missing from legacy Windows OpenGL 1.1 headers
...
// Theme configuration (Light theme default)
...
const char* g_SoundscapeNames[] = { "🔈 Off", "🔈 Garage Rain", "🔈 V8 Rumble", "🔈 Jazz Lounge" };
...
void LoadActiveTexture(const std::string& path) {
    ...
    #ifdef _WIN32
    // Windows wide-character file stream to support absolute UTF-8 local filepaths
    ...
    if (f) {
        data = stbi_load_from_file(f, &g_ActiveTextureWidth, &g_ActiveTextureHeight, &channels, 4);
        fclose(f);
    }
...
// -------------------------------------------------------------
// THEME COMPILER BINDINGS (Neumorphic Soft UI Specs)
// -------------------------------------------------------------
...
void ApplyTheme(Theme theme) {
    ...
    style.WindowRounding = 16.0f;     // Ultra smooth rounded corners
    style.ChildRounding = 12.0f;
    style.FrameRounding = 10.0f;      // Smooth pill buttons & tags
    ...
    if (theme == THEME_LIGHT) {
        // Classic Neumorphic Clay-Grey Styling
        colors[ImGuiCol_Text] = ImVec4(0.24f, 0.29f, 0.37f, 1.00f);        // Deep Slate Obsidian text
        colors[ImGuiCol_WindowBg] = ImVec4(0.88f, 0.90f, 0.93f, 1.00f);    // Clay Gray Background `#E0E5EC`
        colors[ImGuiCol_ChildBg] = ImVec4(0.88f, 0.90f, 0.93f, 1.00f);     // Seamless blending
        colors[ImGuiCol_Border] = ImVec4(0.64f, 0.69f, 0.78f, 1.00f);      // Soft shadow borders
        colors[ImGuiCol_FrameBg] = ImVec4(0.82f, 0.85f, 0.88f, 1.00f);     // Recessed track `#D1D9E6`
        ...
        colors[ImGuiCol_Button] = ImVec4(0.92f, 0.93f, 0.95f, 1.00f);      // Extruded Soft Pill Button `#FAF9F6`
        colors[ImGuiCol_ButtonHovered] = ImVec4(0.84f, 0.86f, 0.90f, 1.00f);
        colors[ImGuiCol_ButtonActive] = ImVec4(0.30f, 0.48f, 1.00f, 1.00f); // Bright Soft Blue `#4D7CFF`
...
// -------------------------------------------------------------
// GRAPHICAL CUSTOM DRAWLIST HELPERS (Pill Progress Bars)
// -------------------------------------------------------------
...
void DrawNeumorphicProgressBar(float fraction, const ImVec2& size, ImU32 barBgColor, ImU32 fillColor) {
    ...
    // Smooth pill-shaped sunken track
    drawList->AddRectFilled(pos, ImVec2(pos.x + size.x, pos.y + size.y), barBgColor, size.y / 2.0f);
    
    // Smooth pill-shaped active fill
    if (fraction > 0.01f) {
        float fillWidth = size.x * fraction;
        drawList->AddRectFilled(pos, ImVec2(pos.x + fillWidth, pos.y + size.y), fillColor, size.y / 2.0f);
    }
    ImGui::Dummy(size);
}
...
int main() {
    ...
    // Global Font Scale - Enlarge default UI components (Bigger Text Constraint)
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (int i = 0; i < (int)g_Catalog.size(); ++i) {
            ...
            // Display beautifully truncated, readable titles instead of robotic first segments
        ...
        if (ImGui::BeginTabBar("DisplayStageTabs")) {
            ...
            if (ImGui::BeginTabItem("Showcase View")) {
                if (g_SelectedCarIndex >= 0 && g_SelectedCarIndex < (int)g_Catalog.size()) {
                    ...
                    drawList->AddRect(canvasCursor, ImVec2(canvasCursor.x + frameSize.x, canvasCursor.y + frameSize.y), IM_COL32(77, 124, 255, 100), 6.0f, 0, 1.5f);

                    // Dynamic secure image loading pipeline (Fuzzy Win32 Wide-character File System Fix)
                    ...
                    if (g_ActiveTextureID != 0) {
                        ...
                        // Blank visual invalid-photo frame as a dark backdrop
        ...
        if (g_SelectedCarIndex >= 0 && g_SelectedCarIndex < (int)g_Catalog.size()) {
            ...
            if (ImGui::BeginTabBar("RightHistologyTabs")) {
                ...
                if (ImGui::BeginTabItem("Nuggets")) {
                    ...
                    // Styled Neumorphic pill-shaped tracker progress bar
                    ImU32 sunkenTrack = IM_COL32(210, 215, 224, 255);
                    ImU32 pillFill = IM_COL32(77, 124, 255, 255); // Blue
                    if (g_ActiveTheme == THEME_NAVY) {
                        sunkenTrack = IM_COL32(15, 23, 42, 255);
                        pillFill = IM_COL32(0, 240, 255, 255); // Neon Cyan
                    } else if (g_ActiveTheme == THEME_BEIGE) {
                        sunkenTrack = IM_COL32(210, 205, 198, 255);
                        pillFill = IM_COL32(184, 115, 51, 255); // Copper
                    }
                    DrawNeumorphicProgressBar(car.collectabilityScore / 10.0f, ImVec2(panelWidth - 50.0f, 14.0f), sunkenTrack, pillFill);
        ...
        else if (g_ActiveTheme == THEME_BEIGE) glClearColor(0.95f, 0.93f, 0.89f, 1.00f);
