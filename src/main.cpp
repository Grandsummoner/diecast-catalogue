...
// Textures, HTTPS, and parsing helpers
void LoadActiveTexture(const std::string& path) {
    if (g_ActiveTexturePath == path) return;
                ...
                if (g_SelectedCarIndex >= 0 && g_SelectedCarIndex < (int)g_Catalog.size()) {
                    ...
                    if (g_ActiveTextureID != 0) {
                        ...
                        if (imgAspect > canvasAspect) {
                            displaySize.x = frameSize.x; displaySize.y = frameSize.x / imgAspect;
                            offset.y = (frameSize.y - displaySize.y) / 2.0f;
                        } else {
                            displaySize.y = frameSize.y; displaySize.x = frameSize.y * imgAspect;
