...
// Dynamic OpenGL Multi-Texture Cache Map (Eviction protected)
...
// Texture Loader, WinHTTP and local calculations
...
GLuint GetOrCreateTexture(const std::string& path) {
    ...
    if (g_TextureCache.size() > 100) {
        auto firstElement = g_TextureCache.begin();
        glDeleteTextures(1, &(firstElement->second));
        g_TextureCache.erase(firstElement);
    }
    GLuint texID = 0;
    int channels, w, h;
    ...
    #endif
    if (f) {
        data = stbi_load_from_file(f, &w, &h, &channels, 4);
        fclose(f);
    }
    if (data) {
        ...
        if (g_SelectedCarIndex >= 0 && g_Catalog[g_SelectedCarIndex].folderPath == path) {
            g_ActiveTextureWidth = w;
            g_ActiveTextureHeight = h;
        }
...
void ExecuteRedo() {
    ...
    HistoryState nextState = g_RedoStack.back(); g_RedoStack.pop_back();
    g_Catalog = nextState.catalog; g_SelectedCarIndex = nextState.selectedCarIndex;
    g_UnsavedChanges = true;
}
...
bool exportWorkspace(const std::string& zipPath) {
    ...
    if (!mz_zip_writer_init_file(&zipArchive, zipPath.c_str(), 0)) return false;
    ...
    mz_zip_writer_finalize_archive(&zipArchive); mz_zip_writer_end(&zipArchive);
    g_UnsavedChanges = false;
    return true;
}

// Directory Drop Scanning logic
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        ImGui::SameLine();
        ImGui::Text("Total: %zu", g_Catalog.size());
        ImGui::SameLine();
        ImGui::SeparatorEx(ImGuiSeparatorFlags_Vertical);
        ImGui::SameLine();
        ...
        // 20% Left panel width, 50% Middle panel width, 30% Right panel width
        ...
        for (int i = 0; i < (int)g_Catalog.size(); ++i) {
            ...
            // Layout cards with rounded corners
            ...
            // Draw a neat Star Toggle inside the card
            ...
            // High-fidelity thumbnail photo rendering inside Left Panel grid card
            GLuint thumbID = GetOrCreateTexture(car.folderPath);
            if (thumbID != 0) {
                ImGui::Image((void*)(intptr_t)thumbID, ImVec2(80, 45));
            } else {
                // Flat silhouette graphic fallback
                ImGui::Text("   🚗   ");
            }
        ...
        if (ImGui::BeginTabBar("DisplayStageTabs")) {
            if (ImGui::BeginTabItem("Showcase View")) {
                if (g_SelectedCarIndex >= 0 && g_SelectedCarIndex < (int)g_Catalog.size()) {
                    ...
                    // Visual Viewport Frame boundaries - Dynamically scaled to be Main Front & Centre
                    ImDrawList* drawList = ImGui::GetWindowDrawList();
                    ImVec2 canvasCursor = ImGui::GetCursorScreenPos();
                    
                    // Maximized 16:9 Canvas Viewport width/height based on Middle Panel Width (Front and Centre)
                    float canvasWidth = middlePanelWidth - 30.0f;
                    float canvasHeight = canvasWidth * (9.0f / 16.0f); // Perfect 16:9 Aspect Ratio
